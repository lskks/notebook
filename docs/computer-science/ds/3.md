...
`insert` 在节点 `p` 之后插入新节点，只需要修改四个指针；`remove` 删除节点 `p`，只需要修改两个指针再释放 `p`
...
双向循环链表的关键优势在于：只要拿到位置的节点指针，插入和删除都是 $O(1)$ 的。单链表中删除节点 `p` 需要先找到 `p` 的前驱，代价是 $O(N)$，而双向链表的每个节点都直接持有前驱指针
...
**队列**(queue)是限制插入和删除分别在表的两端进行的线性表：插入在**队尾**(rear)进行，删除在**队头**(front)进行。因此队列是一个**先进先出**(first in, first out, FIFO)的线性表
...
与栈一样，队列可以用数组实现，但有一个问题：`remove` 之后队头会前进，如果简单地让 `front` 递增，数组前端的空间就被浪费了。解决办法是让数组**循环使用**，即 `front` 和 `rear` 到达数组末尾后回到开头

由于 `front` 和 `rear` 之间既可能"空"也可能"满"，仅凭两者相等无法区分队列状态，需要多留一个位置（或记录元素个数）来区分
...
!!! note "循环队列的容量"
    `isFull` 的条件是 `(rear+1) % MaxSize == front`，即牺牲一个存储单元来区分队空和队满。此时队列最多容纳 `MaxSize - 1` 个元素

### Linked List Implementation

链表实现更简单：用 `front` 和 `rear` 两个指针分别指向队头节点和队尾节点，`add` 在尾部插入、`remove` 在头部删除，都是 $O(1)$
...
!!! warning "别忘了更新 rear"
    当队列中只剩最后一个节点时，`remove` 之后必须把 `rear` 也置为 `nullptr`，否则 `rear` 将成为**悬空指针**，后续 `add` 会向已释放的内存写入
