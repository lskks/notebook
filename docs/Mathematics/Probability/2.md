...
$P(A|B)$被称为**在$B$发生的条件下$A$发生的条件概率**.要在$F$发生的条件下让$E$也发生,实际结果必须同时包含在$E$和 $F$中,即属于$EF$.同时,既然已经知道$F$发生了,那些$F$未发生的结果就不再可能出现了,因此$F$就成了新的,缩小后的样本空间.
根据上面的描述便可以得到条件概率的定义
...
!!! question "Gambler's ruin"
    两个赌徒A和B,进行一系列1$的打赌,其中A获胜的概率是 $p$,B获胜的概率是$q=1-p$.假设A初始有$i$美元,B初始有   $N-i$美元.每次A损失一美元,这一美元就到了B手里,反之亦然.因此,两个人之间的总财富保持不变(即$N$).整个过程如下 图所示
